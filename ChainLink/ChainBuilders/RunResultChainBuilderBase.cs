...
namespace ChainLink.ChainBuilders
{
        ...
        where TChainLink : IRunChainLink<TInput>, IResultChainLink<TResult>
    {
        ...
        new public IRunResultChainBuilder<TResult, TResult, IfChainLink<TResult>> If(Func<bool> condition)
        {
            return If((_, __, cancel) => Task.Run(() => condition(), cancel));
        }

        new public IRunResultChainBuilder<TResult, TResult, IfChainLink<TResult>> If(Func<Task<bool>> condition)
        {
            return If((_, __, ___) => condition());
        }

        new public IRunResultChainBuilder<TResult, TResult, IfChainLink<TResult>> If(Func<IChainLinkRunContext, bool> condition)
        {
            return If((_, context, cancel) => Task.Run(() => condition(context), cancel));
        }

        new public IRunResultChainBuilder<TResult, TResult, IfChainLink<TResult>> If(Func<IChainLinkRunContext, Task<bool>> condition)
        {
            return If((_, context, __) => condition(context));
        }

        new public IRunResultChainBuilder<TResult, TResult, IfChainLink<TResult>> If(Func<IChainLinkRunContext, CancellationToken, bool> condition)
        {
            return If((_, context, cancel) => Task.Run(() => condition(context, cancel), cancel));
        }

        new public IRunResultChainBuilder<TResult, TResult, IfChainLink<TResult>> If(Func<IChainLinkRunContext, CancellationToken, Task<bool>> condition)
        {
            return If((_, context, cancel) => condition(context, cancel));
        }
